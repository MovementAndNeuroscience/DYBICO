...
#%% Open raw data file 
# raw = np.fromfile('C:/Users/dzn332/OneDrive - University of Copenhagen/Documents/DYBICO/DATA/20221108_X08990/20221108_X08990_20221108_14-38-59.raw')
...
class aReader:
    def __init__(self,port=None,nchannels=2,dump=None,ncache=500*60*20,debug=False):
        if port is None:
            ...
            print(ports)
            p=np.nonzero([i[1].find('USB Serial Port')>=0 for i in ports])[0]  # find 'CP210x' if applejack # USB Serial Port
            if len(p)>0:
                p=int(p[0])
                port=ports[p][0]
                print('Port identified %s'%port)
            else :
                p=np.nonzero([i[1].find('USB-SERIAL')>=0 for i in ports])[0]
                if len(p)>0:
                    p=int(p[0])
                    port=ports[p][0]
                    print('Port identified %s'%port)
