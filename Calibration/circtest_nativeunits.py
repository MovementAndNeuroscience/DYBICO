...
import time
pico=aReader.aReader(port='COM6',debug=False,dump='dump.raw')
...
while offset[0][0] < 1 and offset[0][1] < 1: 
    offset = np.mean(pico.getValues(10)[:,1:],axis=0)[None]
...
while not event.getKeys():
    ...
    txt.setText('%i\n(%i,%i)'%(v.mean(),v[0],v[1]))
    txt.draw()
    circ1.setSize(v[0]/600*np.array([1,1])) # 1*np.array([1,1])
    circ1.draw()
    circ2.setSize(v[1]/600*np.array([1,1]))
    circ2.draw()
    mywin.flip()
pico.stop()
del pico
